...
Route::get('/skill_user/{id}', function ($id) {
    ...
    $id = DB::table('skill_user')->updateorinsert(['skill_id' => $id, 'user_id' => Auth::user()->id, 'level' => 3]);
...
Route::get('/skill_user_update/{id_update}', function ($id_update) {

    $id_update = DB::table('skill_user')->update(['skill_id' => $id_update, 'user_id' => Auth::user()->id, 'level' => 5]);
    return redirect()->route('current_user');
});